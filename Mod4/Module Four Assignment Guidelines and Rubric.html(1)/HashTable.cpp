...
void HashTable::Insert(Bid bid) {
  ...
  // assing old node key to UNIT_MAX, set to key, set old node to bid and old
  // node next to null pointer
...
void HashTable::Remove(string bidId) {
  ...
  if (bucket->bid.bidId == bidId) {
    Node *nextNode = bucket->next;
    if (nextNode == nullptr) {
      bucket->key = UINT_MAX;
      bucket->bid = Bid();
    } else {
      bucket->bid = nextNode->bid;
      bucket->key = nextNode->key;
      bucket->next = nextNode->next;
      delete nextNode;
    }
    return;
  }
